...
p<- ggplot(comboData,aes(x=species,y=dS,color=species)) + geom_boxplot(outlier.shape = NA) + geom_jitter(size = 0.6,width=0.2,alpha=0.7,shape=16) + 
        labs(title="Boxplot of Ks Frequency in Duplicated Gene Pairs Across Friedmanniomyces simplex, Friedmanniomyces endolithicus and Hortaea werneckii", x="Species", y="Ks Frequency") + scale_color_brewer(palette='Set1') + theme_bw()
